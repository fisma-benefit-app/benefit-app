...
### 3. On the bottom, it should show 'Status: 200 OK' and 'Body' should contain JSON. 
...
### 4. Copy the raw JWT token (not the whole JSON response) without the quotation (") marks.

### 5. On the authorization tab, change the 'type' to `Bearer token`. Paste the raw token.
...
`In some buggy instances, no JSON will be visible. Simply switch the tab to 'Cookies' and then switch back to 'Body'.`
