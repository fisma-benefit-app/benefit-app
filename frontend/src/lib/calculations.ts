...
const calculateInteractiveEndUserInputService = (
  functionalComponent: TGenericComponent,
) => {
  ...
  const functionalityMultiplier = Number(
    functionalComponent.componentType.split("-")[0],
  );

  return (
    functionalityMultiplier *
    (0.2 + dataElements / 5 + writingReferences / 1.5 + readingReferences / 2)
  );
};

const calculateNonInteractiveEndUserOutputService = (
  functionalComponent: TGenericComponent,
) => {
  const dataElements = functionalComponent.dataElements ?? 0;
  const readingReferences = functionalComponent.readingReferences ?? 0;
...
const calculateInterfaceServiceToOtherApplications = (
  functionalComponent: TGenericComponent,
) => {
  const dataElements = functionalComponent.dataElements ?? 0;
  const readingReferences = functionalComponent.readingReferences ?? 0;
...
const calculateInterfaceServiceFromOtherApplications = (
  functionalComponent: TGenericComponent,
) => {
  const dataElements = functionalComponent.dataElements ?? 0;
  const writingReferences = functionalComponent.writingReferences ?? 0;
...
const calculateDataStorageService = (
  functionalComponent: TGenericComponent,
) => {
  const dataElements = functionalComponent.dataElements ?? 0;
...
const calculateAlgorithmicOrManipulationService = (
  functionalComponent: TGenericComponent,
) => {
  const dataElements = functionalComponent.dataElements ?? 0;
